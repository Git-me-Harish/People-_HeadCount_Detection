...
def _insert_records(
    db: Session, org_id: int, count: int, person_count: int, hour_offset: int = 0
) -> None:
...
def test_insufficient_baseline(db: Session, test_user) -> None:
    result = check_anomaly(
        db, organization_id=test_user.organization_id, camera_id=None, person_count=50
    )
...
def test_normal_reading(db: Session, test_user) -> None:
    ...
    result = check_anomaly(
        db, organization_id=test_user.organization_id, camera_id=None, person_count=21
    )
...
def test_spike_detected(db: Session, test_user) -> None:
    ...
    result = check_anomaly(
        db, organization_id=test_user.organization_id, camera_id=None, person_count=500
    )
