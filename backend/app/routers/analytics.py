...
def summary(
    ...
    if camera_id is not None:
        base_filter.append(DetectionRecord.camera_id == camera_id)
    ...
    if camera_id is not None:
        latest_q = latest_q.filter(DetectionRecord.camera_id == camera_id)
    latest = latest_q.order_by(DetectionRecord.created_at.desc()).first()
    return {
        "window_days": days,
        "camera_id": camera_id,
...
def timeseries(
    ...
    since = datetime.now(timezone.utc) - timedelta(days=days)
    q = db.query(DetectionRecord).filter(
        DetectionRecord.organization_id == user.organization_id,
        DetectionRecord.created_at >= since,
    )
    if camera_id is not None:
        q = q.filter(DetectionRecord.camera_id == camera_id)
    records = q.order_by(DetectionRecord.created_at.asc()).all()
